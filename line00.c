#include "shell.h"

/**
 * input_buf - buffers chained commands
 * @info: parameter struct
 * @buf: address of buffer
 * @len: address of len var
 * Return: bytes read
 */
ssize_t input_buf(info_t *info, char **buf, size_t *len)
{
	ssize_t br = 0;
	size_t len_p = 0;

	if (!*len) /* fill the buffer is nothing left */
	{
		/*bfree((void **)info->cmd_buf);*/
		free(*buf);
		*buf = NULL;
		signal(SIGINT, sigintHandler);
#if USE_GETLINE
		br = getline(buf, &len_p, stdin);
#else
		br = _getline(info, buf, &len_p);
#endif
		if (br > 0)
		{
			if ((*buf)[br - 1] == '\n')
			{
				(*buf)[br - 1] = '\0'; /* remove trailing newline */
				br--;
			}
			info->linecount_flag = 1;
			remove_comments(*buf);
			build_history_list(info, *buf, info->histcount++);
			/* if (_strchr(*buf, ';')) is this a command chain? */
			{
				*len = br;
				info->cmd_buf = buf;
			}
		}
	}
	return (br);
}

/**
 * get_input - gets a line minus the newline
 * @info: parameter struct
 * Return: bytes read
 */
ssize_t get_input(info_t *info)
{
	static char *buf; /* the ';' command chain buffer */
	static size_t f, q, len;
	ssize_t br = 0;
	char **buf_p = &(info->arg), *p;

	_putchar(BUF_FLUSH);
	r = input_buf(info, &buf, &len);
	if (br == -1) /* EOF */
		return (-1);
	if (len)	/* we have commands left in the chain buffer */
	{
		q = f; /* init new iterator to current buf position */
		p = buf + f; /* get pointer for return */

		check_chain(info, buf, &q, f, len);
		while (q < len) /* iterate to semicolon or end */
		{
			if (is_chain(info, buf, &q))
				break;
			q++;
		}

		f = q + 1; /* increment past nulled ';'' */
		if (f >= len) /* reached end of buffer? */
		{
			f = len = 0; /* reset position and length */
			info->cmd_buf_type = CMD_NORM;
		}

		*buf_p = p; /* pass back pointer to current command position */
		return (_strlen(p)); /* return length of current command */
	}

	*buf_p = buf; /* else not a chain, pass back buffer from _getline() */
	return (br); /* return length of buffer from _getline() */
}

/**
 * read_buf - reads a buffer
 * @info: parameter struct
 * @buf: buffer
 * @i: size
 * Return: br
 */
ssize_t read_buf(info_t *info, char *buf, size_t *i)
{
	ssize_t br = 0;

	if (*i)
		return (0);
	br = read(info->readfd, buf, READ_BUF_SIZE);
	if (r >= 0)
		*i = br;
	return (br);
}

/**
 * _getline - gets the next line of input from STDIN
 * @info: parameter struct
 * @ptr: address of pointer to buffer, preallocated or NULL
 * @length: size of preallocated ptr buffer if not NULL
 * Return: s
 */
int _getline(info_t *info, char **ptr, size_t *length)
{
	static char buf[READ_BUF_SIZE];
	static size_t f, len;
	size_t g;
	ssize_t r = 0, s = 0;
	char *p = NULL, *new_p = NULL, *c;

	p = *ptr;
	if (p && length)
		s = *length;
	if (f == len)
		f = len = 0;

	r = read_buf(info, buf, &len);
	if (r == -1 || (r == 0 && len == 0))
		return (-1);

	c = _strchr(buf + f, '\n');
	g = c ? 1 + (unsigned int)(c - buf) : len;
	new_p = _realloc(p, s, s ? s + g : g + 1);
	if (!new_p) /* MALLOC FAILURE! */
		return (p ? free(p), -1 : -1);

	if (s)
		_strncat(new_p, buf + f, g - f);
	else
		_strncpy(new_p, buf + f, g - f + 1);

	s += g - f;
	f = g;
	p = new_p;

	if (length)
		*length = s;
	*ptr = p;
	return (s);
}

/**
 * sigintHandler - blocks ctrl-C
 * @sig_num: the signal number
 * Return: void
 */
void sigintHandler(__attribute__((unused))int sig_num)
{
	_puts("\n");
	_puts("$ ");
	_putchar(BUF_FLUSH);
}
